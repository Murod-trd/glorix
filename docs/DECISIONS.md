...
## Decision 12: Buying-in-the-Marketplace and Creating-a-Tender Are Separate Permissions

**Decision**: `AccountContext.jsx` exposes three independent permission flags instead of the old two: `canCreateTender` (buyer/both), `canBuyMarketplace` (all three account types, including seller), and `canSell` (seller/both, unchanged). `canBuy` is kept as a backward-compatible alias for `canCreateTender`. `Marketplace.jsx` now gates its buy UI (cart, purchase form, buy button) on `canBuyMarketplace` instead of the old `canBuy`, and a seller account gets a "Мои товары"/"Весь каталог" toggle to switch between viewing only its own listings and the full purchasable catalog.

**Rationale**: The founder clarified that, in real B2B trade, a seller commonly also needs to purchase raw materials or components for its own production -- that's a marketplace-buying need, completely independent of whether the account can create a buyer-side tender. The previous single `canBuy` flag conflated these two distinct permissions, which meant a seller account could never buy anything in the marketplace at all, even materials for itself. Splitting the flag is a direct, literal implementation of that clarification, not a reinterpretation.

**Constraint for future work**: Don't reintroduce a single combined "can this role transact" flag -- any new feature that's marketplace-purchase-specific should check `canBuyMarketplace`, and anything tender-creation-specific should check `canCreateTender`. The `canBuy` alias exists only so already-correct buyer-vs-seller logic (e.g. "show the create-tender button") didn't need a forced rename in the same commit that introduced the split -- new code should use the named flags directly, not `canBuy`.

---

## Decision 13: Tenders Visible to Every Role; Buyer Identity Anonymized Until Tender Completion

**Decision**: `getAllTenders()` (in the new `src/data/tenderStore.js`) returns every tender -- static demo data plus user-published ones -- to every account type, with no ownership-based filtering on the `/tenders` list or detail pages. Separately, `getDisplayBuyerName(tender, viewerId)` shows the real buyer company name only to the buyer themselves (`tender.buyerId === viewerId`) or once the tender's `status` is `'completed'`; otherwise it returns an anonymized "Покупатель из [country]" label to every other viewer, including sellers deciding whether to bid.

**Rationale**: Reached after the founder explicitly asked for business-consultant-style advice on how the platform should handle buyer/seller visibility for tenders. The chosen model mirrors how real B2B RFQ platforms (Alibaba RFQ, SAP Ariba) work: sellers need to see the full open tender flow to find things worth bidding on, but buyers reasonably don't want their company identity broadcast to every potential bidder before they've chosen who to deal with -- premature identity exposure can affect pricing and negotiating leverage. Country-level anonymization keeps the listing useful (sellers can still judge logistics/incoterms feasibility) without revealing the specific company.

**Known data gap, left open rather than papered over**: none of the three static demo tenders (`mock.js`) were ever linked to a specific demo buyer account (no `buyerId`). t1 (destination country UZ) and t3 (RU) were explicitly linked to the buyer and "both" demo personas (`u1`, `u3`) since the countries match cleanly. t2 (KZ) has no valid match -- the only Kazakhstani demo account is the seller persona (KazSteel Trading, `u2`), and linking a tender's `buyerId` to an account that, per Decision 12, cannot create tenders would be an internal contradiction baked into the demo data. `t2.buyerId` is deliberately left unset, so it displays as anonymous to every viewer including its own "owner" (who doesn't really exist). This mirrors the project's established practice (see the marketplace `getAllProducts`/seller-linking fix) of fixing what can be fixed cleanly and explicitly flagging what can't, rather than forcing a fake link or silently leaving it inconsistent.

**Constraint for future work**: Any newly created demo account, or a real backend account system, should populate `buyerId` at tender-creation time (already done for `addUserTender()` -- it sets `buyerId: currentUser.id`). Don't backfill `t2.buyerId` with an incorrect persona just to make the field non-null; either add a real Kazakhstani buyer demo persona (founder decision) or leave it anonymous permanently.
