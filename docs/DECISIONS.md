...
## Decision 10: HS Code (ТН ВЭД) Search — Real Dataset + Unofficial Google Translate Endpoint + Dictionary Fallback

**Decision**: TNVED search is layered three ways, in priority order: (1) direct match against the real official Harmonized System dataset (5,613 codes, English descriptions, from `datasets/harmonized-system` / UN Comtrade, ODC PDDL license) for code or English-term queries; (2) for Russian queries, an instant local dictionary (~150+ common CIS B2B trade terms) translates to English keywords with zero network dependency; (3) if the dictionary has no entry, the unofficial `translate.googleapis.com` endpoint (no API key, undocumented, not officially supported by Google) is called as a live fallback, and the platform shows the user multiple candidate code matches to choose from rather than picking one silently.

**Rationale**: The founder explicitly asked for broad Russian-language coverage beyond what a hand-curated dictionary can practically achieve, and approved using a free external translator plus expanding the dictionary, with backend infrastructure accepted as the eventual real fix (Roadmap MVP/Beta phases already plan a real backend). The unofficial Google endpoint was chosen over self-hosted LibreTranslate because the platform currently has no backend to run a translation server on — running entirely client-side is the only option compatible with the Demo phase's no-backend constraint. This is a deliberate, founder-approved tradeoff, not a silent shortcut: the endpoint is undocumented and Google could change or block it without notice (multiple sources confirm this), so the dictionary layer must remain functional as a no-network fallback, and the live-translate layer must fail gracefully (show "translation unavailable, try an English term" rather than crash) rather than being treated as guaranteed-available infrastructure.

**Constraint for future work**: When the platform reaches MVP/Beta and gains a real backend, this unofficial endpoint should be replaced with either a paid/official translation API (Google Cloud Translation, DeepL) or a self-hosted LibreTranslate instance — both require server-side infrastructure this phase doesn't have. Don't silently keep relying on the unofficial endpoint once a backend exists; it was a Demo-phase-only tradeoff, not a permanent architecture choice.
