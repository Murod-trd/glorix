...
function TenderRow({ tender, onClick }) {
  ...
  return (
    ...
    >
        ...
        <div style={{ display: 'flex', gap: 4, justifyContent: 'flex-end' }}>
          {[1,2,3,4,5].map(i => (
            <div key={i} style={{
              ...
              background: i <= progress ? 'var(--accent)' : 'var(--navy-3)',
      ...
      <div style={{ fontSize: 13, color: 'var(--text-2)', minWidth: 50, textAlign: 'right' }}>
        {tender.offers} офертa
      </div>
    </div>
  );
}
  ...
  return (
    <div className="fade-in" style={{ padding: '32px 36px' }}>
      ...
      <div style={{ display: 'flex', gap: 16, marginBottom: 32, flexWrap: 'wrap' }}>
        ...
        <StatCard value={`${stats.avgTrustScore}%`} label="Средний рейтинг доверия" />
      </div>
      ...
      <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', overflow: 'hidden', marginBottom: 28 }}>
        <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ fontWeight: 600, fontFamily: 'var(--font-display)' }}>Мои тендеры</div>
          <button className="btn btn-ghost" onClick={() => navigate('/tenders')} style={{ fontSize: 13, padding: '6px 14px' }}>
            Все тендеры →
          </button>
        </div>
        {tenders.map(t => (
          <TenderRow key={t.id} tender={t} onClick={() => navigate(`/tenders/${t.id}`)} />
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        ...
        <div className="card" style={{ borderColor: 'rgba(0,212,170,0.2)', background: 'rgba(0,212,170,0.04)' }}>
          <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
            ...
            <div>
              ...
              <button className="btn btn-ghost" onClick={() => navigate('/ai-analysis')} style={{ marginTop: 12, fontSize: 13, padding: '7px 14px' }}>
                Смотреть анализ →
              </button>
            </div>
          </div>
        ...
        <div className="card" onClick={() => navigate('/trust')} style={{ cursor: 'pointer' }}>
          ...
          <div style={{ fontSize: 40, fontWeight: 700, fontFamily: 'var(--font-display)', color: 'var(--accent)', marginBottom: 8 }}>
