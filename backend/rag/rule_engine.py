...
class RuleEngine:
    ...
    def _opi3b(self) -> RuleResult:
        ...
        ЧАСТИЧНО РЕАЛИЗОВАНО: обнаруживает составной товар по маркерам.
        НЕ РЕАЛИЗОВАНО: определение компонента с существенным характером
        (требует стоимостных данных, весов, функциональной оценки).
        ...
        return RuleResult(
            ...
            checks_performed=checks,
            reason=(
                f"Обнаружены маркеры составного изделия: {found}. "
                "ОГРАНИЧЕНИЕ: для применения ОПИ 3б необходимо определить, какой компонент "
                "придаёт товару существенный характер. "
                "Это требует стоимостных данных, функциональной оценки или весов компонентов — "
                "автоматическое применение невозможно. "
                "ДЕЙСТВИЕ: обратитесь к таможенному декларанту."
            ),
            evidence=[f"Маркер: '{m}'" for m in found],
        )
