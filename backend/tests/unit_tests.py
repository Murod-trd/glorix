...
from rag.opi_checker import run_opi_checks  # deprecated stub — DeprecationWarning
...
def _():
    ...
    import warnings
    from rag.opi_checker import OPIReport
...
from rag.devil_advocate import (
...
def _():
    ...
    from tests.benchmark import BenchmarkRun
    for case in cases:
    ...
    from tests.benchmark import BenchmarkReport
...
# ═══════════════════════════════════════════════════════════════════════
from rag.product_feature_extractor import extract_features, ProductFeatures, MATERIAL_DOMINANT_FUNCTIONS
...
# RuleEngine Tests
# ═══════════════════════════════════════════════════════════════════════
from rag.rule_engine import RuleEngine, RuleVerdict, RuleResult, RuleEngineReport
...
# ═══════════════════════════════════════════════════════════════════════
from rag.validator import validate_classification, ValidationResult
...
def _():
    ...
    assert len(opi3b) == 1
    # v7 FIX: теперь возвращает CONFIRMS (HEURISTIC) вместо INSUFFICIENT
    assert opi3b[0].verdict == RuleVerdict.CONFIRMS, (
        "v7: ОПИ 3б должен вернуть CONFIRMS (HEURISTIC) для 'набор'. Получено: " + str(opi3b[0].verdict)
    )
    assert opi3b[0].is_heuristic is True, "ОПИ 3б CONFIRMS должен быть помечен is_heuristic=True"
...
def _():
    from config import MIN_CONFIDENCE_TO_ANSWER as cfg_val
    from rag.validator import MIN_CONFIDENCE_TO_ANSWER as val_val
