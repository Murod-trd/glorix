...
def extract_pdf(filepath: str | Path) -> list[PdfChunk]:
    ...
    logger.info(
        ...
        "(с главой: %d, без главы: %d)",
        source_name, len(chunks), page_count, chunks_with_chapter, chunks_without_chapter,
    )
    if chunks_without_chapter > 0:
        logger.warning(
            "[PDFExtractor] %s: %d чанков без определённой главы ТН ВЭД — "
            "возможна потеря контекста при поиске по главе",
            source_name, chunks_without_chapter,
        )
    return chunks
...
def extract_all_pdfs(pdf_dir: str | Path) -> list[PdfChunk]:
    ...
    logger.info("[PDFExtractor] Найдено {len(pdf_files)} PDF файлов")
    ...
    for pdf_file in pdf_files:
        try:
            ...
            logger.info("[PDFExtractor] ОШИБКА {pdf_file.name}: {e}")

    logger.info("[PDFExtractor] Итого: {len(all_chunks)} чанков из {len(pdf_files)} файлов")
    return all_chunks
...
def _build_chunks(paragraphs: list[dict], source_file: str) -> list[PdfChunk]:
    ...
    for para in paragraphs:
        ...
        # HEURISTIC: Распознать заголовок главы ТН ВЭД по шаблону "Глава N" + жирный шрифт.
        # Ложные срабатывания возможны если "Глава" встречается в тексте примечаний.
        ...
        # HEURISTIC: Распознать раздел ТН ВЭД по шаблону "Раздел N" + жирный + короткий.
